...
* **Flask Endpoints:** Проверка создания задач и корректной обработки некорректных запросов.

  * Файлы: `test_flask_unit.py`
  * Примеры проверок:

    * Успешное добавление задачи через `/add`
    * Валидация неполных данных (ожидаем ошибку 400/422)

* **Cloud Function:** Проверка логики обновления статуса задачи.

  * Файл: `test_cloud_function.py`
  * Примеры проверок:

    * Обновление статуса задачи на `done` или `not done`
    * CORS preflight запросы (OPTIONS)
    * Обновление нескольких задач

* **Запуск тестов:**

```bash
pytest -s test_flask_unit.py
...
* **Cloud Function + Datastore:** Проверка полной цепочки взаимодействия.

  * Файл: `test_integration.py`
  * Сценарии:

    * Создание задачи напрямую в Datastore и обновление статуса через Cloud Function
    * Создание нескольких задач и обновление статуса одной из них
* **Цель:** Убедиться, что данные корректно сохраняются и обновляются на уровне всей инфраструктуры.
* **Запуск тестов:**
...
* **Сценарий:** Имитируются тысячи одновременных пользователей, выполняющих:

  * просмотр задач
  * создание задач
  * (опционально) обновление статуса через Cloud Function
* **Анализ:** Отслеживаются **Requests per second (RPS)** и **среднее время ответа (Latency)** для оценки производительности App Engine и Cloud Functions при пиковой нагрузке.
* **Запуск тестирования:**
